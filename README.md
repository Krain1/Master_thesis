...
flowchart TD
  %% ============ LAYOUT ============
  %% Top-to-bottom layout keeps things readable in README
  %% =================================

  %% ---------- SETUP ----------
  subgraph A[Setup & Profile Preparation]
    direction TB
    A1[Mount Google Drive]
    A2[Install dependencies<br/>(pip + playwright)]
    A3[Configure LLM client<br/>OPENAI_API_KEY • API_BASE • model]
    A4[Load All_profiles.xlsx]
    A5[Translate traits → English]
    A6[Generate reflections<br/>profile • actual • ideal]
    A7[Init EnhancedMemoryModule<br/>Vector LTM • STM • Episodic • CoT<br/>Store profile & reflections]
    A1 --> A2 --> A3 --> A4 --> A5 --> A6 --> A7
  end

  %% ---------- SURVEY ----------
  subgraph B[Survey Orchestration (AgentSurveyOrchestrator)]
    direction TB
    B1[Pick brand ∈ {Nike • Apple • Levi's}]
    B2[Pick question ∈ {Q9 • Q10 • Q11}]
    B3[Initial reasoning (LLM)<br/>→ Rating I + Confidence]
    B4{Confidence &lt; 7.5<br/>or first brand?}
    B5[BrandPersonalityToolOrchestrator<br/>Critical description + Big Five<br/>+ negative aspects]
    B6{Challenge needed?}
    B7[Integrate challenge<br/>Revise reasoning → RATING II]
    B8[Finalize answer]
    B9[Apply learning adjustment<br/>(SharedLearning) ±1 step if |avg_diff| ≥ 0.3]
    B10[Store survey rating + reasoning<br/>STM • Episodic (if high attach.) • CoT]
    B1 -- for each brand --> B2
    B2 -- for each question --> B3 --> B4
    B4 -- Yes --> B5 --> B6
    B6 -- Yes --> B7 --> B8
    B6 -- No  --> B8
    B4 -- No  --> B8
  ...
  %% ---------- BIDDING ----------
  subgraph C[Bidding Orchestration (AgentBiddingOrchestrator)]
    direction TB
    C1[Read product from shop UI]
    C2[PriceResearchToolOrchestrator<br/>Reference price (scrape/fallback)]
    C3[Fetch brand bidding adjustment<br/>(SharedLearning)]
    C4[CoT bidding using cross-brand memories]
    C5[Consistency check (LLM)]
    C6[Compute final bid<br/>apply adjustment • min €40]
    C7[Playwright submits bid to site]
    C8[Store bidding reasoning in memory]
    C1 --> C2 --> C3 --> C4 --> C5 --> C6 --> C7 --> C8
  end

  %% ---------- FEEDBACK & LEARNING ----------
  subgraph D[Feedback & Learning Loop]
    direction TB
    D1[Find top-3 human matches<br/>(semantic similarity on traits & demographics)]
    D2[Aggregate survey & bid differences]
    D3[Create recommendations<br/>survey tweaks • bidding offsets]
    D4[Update Shared Learning Memory<br/>survey_adjustments • bidding_adjustments • wisdom]
    D1 --> D2 --> D3 --> D4
  end

  %% ---------- OUTPUTS ----------
  subgraph E[Outputs → Google Drive]
    direction TB
    E1[CSV/XLSX/JSON exports<br/>survey • reasoning • bids • comms]
    E2[Checkpoint saves every 10 agents]
    E3[Final summaries & statistics]
    E1 --> E2 --> E3
  end
  ...
  %% ---------- STYLES ----------
  classDef setup   fill:#f1f8ff,stroke:#0366d6,stroke-width:1px,color:#0b3d91;
  classDef survey  fill:#fff7ed,stroke:#f97316,stroke-width:1px,color:#7c2d12;
  classDef bidding fill:#ecfeff,stroke:#06b6d4,stroke-width:1px,color:#134e4a;
  classDef fb      fill:#f0fdf4,stroke:#22c55e,stroke-width:1px,color:#14532d;
  classDef out     fill:#fef2f2,stroke:#ef4444,stroke-width:1px,color:#7f1d1d;
  classDef decision fill:#ffffff,stroke:#111827,stroke-width:1px,stroke-dasharray:5 3,color:#111827;

  class A1,A2,A3,A4,A5,A6,A7 setup;
  class B1,B2,B3,B5,B7,B8,B9,B10 survey;
  class B4,B6 decision;
  class C1,C2,C3,C4,C5,C6,C7,C8 bidding;
  class D1,D2,D3,D4 fb;
  class E1,E2,E3 out;
